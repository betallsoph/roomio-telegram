<script lang="ts">
	...
	async function loadWorkspace() {
		...
		try {
			...
			if (!propertiesRes.ok)
				throw new Error(propertiesData.error || 'Không tải được danh sách cơ sở');
		...
		<section class="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
			<div class="space-y-5">
				...
				<div>
					...
					{#if rooms.length === 0}
						<div
							class="border-2 border-black bg-white px-4 py-8 text-center text-sm font-bold text-zinc-500"
						>
